...
int PlayLayer::calculateAngle(Point _point1, Point _point2)
{
    int _y = _point2.y - _point1.y;
    int _x = _point2.x - _point1.x;
    if (_x == 0) _x = 1;
    if (_y == 0) _y = 1;
    // goc phan tu thu nhat
    if ((_point2.x >= _point1.x) && (_point2.y >= _point1.y))
    {     
        float _tanAng = abs(_x)/abs(_y);
        float ang = atan(_tanAng) * 180 /3.14;
        return (int)ang + 30;
    }
    // goc phan tu thu 2
    else if((_point2.x < _point1.x) && (_point2.y > _point1.y))
    {
        float _tanAng = abs(_x) / abs(_y);
        float ang = (atan(_tanAng) * 180)/3.14;
        return -(int)ang - 30;
    ...
    {
        float _tanang = abs(_y) / abs(_x);
        return -(((int)(_tanang * 180))/3.14 +90) - 30;
    }
    // goc phan tu thu 4
    else if ((_point2.y < _point1.y) && (_point2.x > _point1.x))
    {
        float _tanang = abs(_y) / abs(_x);
        return 90 + 30 + ((int)(_tanang * 180))/3.14;
    }
    ...
    {   
        ...
        else
        {
            if (auto enemy = m_EnemyArray[i]->getEnemyCharacter())
            {
                ...
                enemy->runAction(m_EnemyArray[i]->getEnemyDieSprite());
                enemy->runAction(Sequence::create(m_EnemyArray[i]->getEnemyDieSprite(),DelayTime::create(0.5), CallFunc::create([=]() {enemy->removeFromParent(); }), NULL));
                CCLOG("action: %d", enemy->getNumberOfRunningActions());
...
void PlayLayer::checkDistanceArcherTower(float dt)
{
    for (int i = 0; i < m_ArcherTowerList.size(); i++)
    {
        ...
        {
            auto _enemyPos = m_EnemyArray[_idxEnemy]->getEnemyCharacter()->getPosition();

            m_EnemyArray[_idxEnemy]->setHealth(m_EnemyArray[_idxEnemy]->getHealth() - 30); 

            auto arrow = m_ArcherTowerList[i]->getArrowOfArcherTowerSprite();
            this->addChild(arrow);

            auto _ang = calculateAngle(_sourcePos, _enemyPos);
            arrow->setRotation(_ang);
            arrow->runAction(Sequence::create(MoveTo::create(0.3, _enemyPos),DelayTime::create(1), FadeOut::create(0.3), nullptr));
        ...
        else
        {
            for (int j = 0; j < m_EnemyArray.size(); j++)
            {
                ...
                if (_enemy)
                {
                    if (_sourcePos.distance(m_EnemyArray[j]->getEnemyCharacter()->getPosition()) < 200)
                    {
                        auto _enemyPos = m_EnemyArray[j]->getEnemyCharacter()->getPosition();
                        ...
                        auto _ang = calculateAngle(_sourcePos, _enemyPos);
                        arrow->setRotation(_ang);
                        arrow->runAction(Sequence::create(MoveTo::create(0.3, _enemyPos), DelayTime::create(1), FadeOut::create(0.1), nullptr));
        ...
        for (int j = 0; j < m_EnemyArray.size(); j++)
        {
            if (m_EnemyArray[j]->getEnemyCharacter() != NULL)
            {
                if (_sourcePos.distance(m_EnemyArray[j]->getEnemyCharacter()->getPosition()) < 200)
                {
                    ...
                    m_EnemyArray[j]->setHealth(m_EnemyArray[j]->getHealth() - 50);

                    auto _magicTower = m_MagicTowerList[i]->getMagicTowerElectricSprite();
                    auto magicPos = m_MagicTowerList[i]->getHeadMagicTowerSprite()->getPosition();
                    auto enemyPos = m_EnemyArray[j]->getEnemyCharacter()->getPosition();
                    _magicTower->setPosition(magicPos);
                    auto _ang = calculateAngle(magicPos, enemyPos);
                    _magicTower->setRotation(_ang);
                    _magicTower->runAction(RepeatForever::create(m_MagicTowerList[i]->getMagicTowerAnimate()));

                    _magicTower->runAction(Sequence::create(MoveTo::create(0.05, enemyPos), DelayTime::create(0.3), RemoveSelf::create(), nullptr));
                    

                    this->addChild(_magicTower);
